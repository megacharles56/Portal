...
use app\models\EstructuraOrganicaSearch;
...
use app\models\VestructuraOrganicaSearch;

/**
 * EstructuraOrganicaController implements the CRUD actions for EstructuraOrganica model.
 */
class EstructuraOrganicaController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'EstructuraOrganica',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
                        'actions' => ['index', 'view', 'add', 'update', 'delete', 'create', 'permisos',
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all EstructuraOrganica models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $model = new EstructuraOrganica();

        $searchModel = new VestructuraOrganicaSearch();
    ...
    /**
     * Displays a single EstructuraOrganica model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new EstructuraOrganica model.
    ...
    /**
     * crea en batch nuevosEstructuraOrganica modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['EstructuraOrganica/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['EstructuraOrganica/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing EstructuraOrganica model.
    ...
    public function actionUpdate($id) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->estor_id]);
            }
        }
    ...
    /**
     * Deletes an existing EstructuraOrganica model.
    ...
    /**
     * Finds the EstructuraOrganica model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return EstructuraOrganica the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = EstructuraOrganica::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function actionChart() {
        ...
        if ($p['EstructuraOrganica']['estor_id']) {
            $estor_id = $p['EstructuraOrganica'] ['estor_id'];
        } else
        /* @todo deberia buscar el primero activo  */
            $estor_id = 1;
        return $this->render('chart', ['estor_id' => $estor_id]);
    }
