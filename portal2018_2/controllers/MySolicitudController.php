...
use app\models\MySolicitudSearch;
...
/**
 * MySolicitudController implements the CRUD actions for MySolicitud model.
 */
class MySolicitudController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'MySolicitud',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all MySolicitud models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new MySolicitudSearch();
    ...
    /**
     * Displays a single MySolicitud model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new MySolicitud model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new MySolicitud();
    ...
    /**
     * crea en batch nuevosMySolicitud modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['MySolicitud/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['MySolicitud/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing MySolicitud model.
    ...
    /**
     * Deletes an existing MySolicitud model.
    ...
    /**
     * Finds the MySolicitud model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return MySolicitud the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = MySolicitud::findOne($id)) !== null) {
            return $model;
        }
