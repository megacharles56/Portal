...
/**
 * SalonesController implements the CRUD actions for Salones model.
 */
class SalonesController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'Salones',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all Salones models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new SalonesSearch();
    ...
    /**
     * Displays a single Salones model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * crea en batch nuevosSalones modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['Salones/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['Salones/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing Salones model.
    ...
    /**
     * Deletes an existing Salones model.
    ...
    /**
     * Finds the Salones model based on its primary key value.
    ...
    protected function findModel($id) {
        if (($model = Salones::findOne($id)) !== null) {
            return $model;
        }
