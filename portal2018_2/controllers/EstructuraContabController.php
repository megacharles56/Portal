...
use app\models\EstructuraContabSearch;
...
/**
 * EstructuraContabController implements the CRUD actions for EstructuraContab model.
 */
class EstructuraContabController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'EstructuraContab',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all EstructuraContab models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new EstructuraContabSearch();
    ...
    /**
     * Displays a single EstructuraContab model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new EstructuraContab model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new EstructuraContab();
        if ($model->load(Yii::$app->request->post())) {
            ...
            print_r($model);
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->estco_id]);
    ...
    /**
     * crea en batch nuevosEstructuraContab modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['EstructuraContab/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['EstructuraContab/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing EstructuraContab model.
    ...
    /**
     * Deletes an existing EstructuraContab model.
    ...
    /**
     * Finds the EstructuraContab model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return EstructuraContab the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = EstructuraContab::findOne($id)) !== null) {
            return $model;
        }
