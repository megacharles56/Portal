...
use app\models\NolaboralesSearch;
...
/**
 * NolaboralesController implements the CRUD actions for Nolaborales model.
 */
class NolaboralesController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'Nolaborales',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all Nolaborales models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new NolaboralesSearch();
    ...
    /**
     * Displays a single Nolaborales model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new Nolaborales model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new Nolaborales();
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->nolab_id]);
    ...
    /**
     * crea en batch nuevosNolaborales modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['Nolaborales/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['Nolaborales/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing Nolaborales model.
    ...
    /**
     * Deletes an existing Nolaborales model.
    ...
    /**
     * Finds the Nolaborales model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return Nolaborales the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = Nolaborales::findOne($id)) !== null) {
            return $model;
        }
