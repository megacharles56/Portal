...
use Yii;
use app\models\TiposArticulo;
use app\models\TiposArticuloSearch;
...
/**
 * TiposArticuloController implements the CRUD actions for TiposArticulo model.
 */
class TiposArticuloController extends Controller {
    ...
    public function ifcanIndex($action, &$nivel, $model = null) {
        ...
        $puede = Vpermisos::findBySql(
                        ...
                        . ' and ( permi_metodo = :action or permi_metodo = ' . "'*'" . ')', ['clase' => 'TiposArticulo',
                    'usr' => $usr,
                    'action' => $action])->all();
        $r = false;
        if (sizeof($puede) > 0) {
...
//$dataProvider->query->andFilterWhere(['event_id' => $id,]);
    ...
    /**
     * Lists all TiposArticulo models.
     * @return mixed
     */
    public function actionIndex() {
        $this->canIndex('index', $nivel);
        $searchModel = new TiposArticuloSearch();
    ...
    /**
     * Displays a single TiposArticulo model.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Creates a new TiposArticulo model.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionCreate() {
        $this->can('create');
        $model = new TiposArticulo();
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                return $this->redirect(['view', 'id' => $model->tiart_id]);
    ...
    /**
     * crea en batch nuevosTiposArticulo modelos hijos de una table.
     * If creation is successful, the browser will be redirected to the 'view' page.
     * @return mixed
     */
    public function actionAddBatch($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                return $this->redirect(['TiposArticulo/add', 'regis' => $ri]);
            } else {
    ...
    public function actionAdd($idMaster) {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save())
                return $this->redirect(['TiposArticulo/view', 'id' => $model->perca_id]);
        }
        else {
    ...
    /**
     * Updates an existing TiposArticulo model.
    ...
    /**
     * Deletes an existing TiposArticulo model.
    ...
    /**
     * Finds the TiposArticulo model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return TiposArticulo the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id) {
        if (($model = TiposArticulo::findOne($id)) !== null) {
            return $model;
        }
